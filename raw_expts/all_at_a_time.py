...
dst_path = '/speech/nishanth/clean_research/ptdb_full_data/mag_final_data_8khz'
base_path = '/speech/nishanth/clean_research/ptdb_full_data/raw_8khz_final'
...
for s in splits:
    if s != "test":
        ...
        for file in delays:
            res = np.load(os.path.join(f"{dst_path}/{s}/raw", file))
            #res = res[0,:,:]
            np.save(os.path.join(f"{dst_path}/{s}/raw", file), res)
            get_label(file)
    else:
        for n in noises:
            ...
            os.makedirs(out_path, exist_ok = True)
            for file in delays:
                res = np.load(os.path.join(f"{dst_path}/{s}/{n}/raw", file))
                #res = res[0,:,:]
                np.save(os.path.join(f"{dst_path}/{s}/{n}/raw", file), res)
                get_label(file)
...
for s in splits:
    if s != "test":
        ...
        for file_name in label_files:
            ...
            len_diff = gd_arr.shape[0] - labels.shape[0]
            if gd_arr.shape[0] > labels.shape[0]:
                new_gd_arr = gd_arr[:labels.shape[0], :]
                np.save(os.path.join(dst_path, s ,"raw", file_name), new_gd_arr)
            else:
        ...
        for q in noises:
            ...
            for file_name in label_files:
                ...
                if gd_arr.shape[0] > labels.shape[0]:
                    new_gd_arr = gd_arr[:labels.shape[0], :]
                    np.save(os.path.join(dst_path, s ,q,"raw", file_name), new_gd_arr)
                else:
                    new_gd_arr = np.concatenate((gd_arr, np.zeros((labels.shape[0] - gd_arr.shape[0], 511))), axis=1)
                    np.save(os.path.join(dst_path,s, q, "raw", file_name), new_gd_arr)                            

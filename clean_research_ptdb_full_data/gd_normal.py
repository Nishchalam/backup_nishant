...
def compute_gd(tup):
    ...
    try:
        ...
        # root_array[~indices] = root_array[~indices]
        # gd_spectrum = [np.zeros_like(root_array) for _ in range(len(radii))]
        gd_spectrum = np.zeros((root_array.shape[0], 512))
        for i in range(root_array.shape[0]): # per frame
            roots = root_array[i,:]
            gd_sum= np.zeros(512)
            for r in roots:
                gd = signal.group_delay(([1, -r], [1]), w=1024)[1]
                # print(gd)
                gd = np.array(gd[:512])
                gd_sum = gd +gd_sum
            gd_spectrum[i, :] = gd_sum
        print(gd_spectrum.shape)
        np.save(out_path, gd_spectrum)# Your compute_gd function code here
    except Exception as e:
...
def main():
    ...
    noise_db_list = ["0","10","5","20","clean"]
    splits = ["train","valid","test"]
    roots_dir_path = "/speech/nishanth/clean_research/ptdb_full_data/roots"
    gd_dir = "/speech/nishanth/clean_research/ptdb_full_data/group_delay_normal"
    nj = 10
    pool = mp.Pool(processes=nj)
    ...
    compute_gd_partial = partial(compute_gd)

...
train_X = os.listdir(f"{data_path}/train/gd_thres")
train_X = [os.path.join(f"{data_path}/train/gd_thres", x) for x in train_X]
train_y = os.listdir(f"{data_path}/train/labels")
train_y = [os.path.join(f"{data_path}/train/labels", y) for y in train_y]

valid_X = os.listdir(f"{data_path}/valid/gd_thres")
valid_X = [os.path.join(f"{data_path}/valid/gd_thres", x) for x in valid_X]
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        return logits
d_model = 256 # Set this to your desired model dimension
...
cross_entropy_loss_weights = torch.full((300,), 1.5)
...
dump_dir = "/speech/nishanth/clean_research/ptdb_full_data/mudit_4_layer_pos_gd_1.008"
os.makedirs(dump_dir, exist_ok=True)
last_model_save_epoch = 0

model_name = "weighted_CEL_one_radius_adam_4_layer_pos_gd_1.008_full"
